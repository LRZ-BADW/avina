...
pub async fn select_maybe_flavor_quota_by_user_and_group_from_db(
    transaction: &mut Transaction<'_, MySql>,
    user_id: u64,
    flavor_group_id: u64,
) -> Result<Option<FlavorQuota>, UnexpectedOnlyError> {
...
pub async fn select_flavor_quota_by_user_and_group_from_db(
    transaction: &mut Transaction<'_, MySql>,
    user_id: u64,
    flavor_group_id: u64,
) -> Result<FlavorQuota, NotFoundOrUnexpectedApiError> {
    select_maybe_flavor_quota_by_user_and_group_from_db(
        transaction,
        user_id,
        flavor_group_id,
    )
...
/// Delete the flavor quota with the given ID from the database.
#[tracing::instrument(name = "delete_flavor_quota_from_db", skip(transaction))]
pub async fn delete_flavor_quota_from_db(
    transaction: &mut Transaction<'_, MySql>,
    flavor_quota_id: u64,
