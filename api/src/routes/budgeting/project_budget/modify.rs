...
use crate::{
    ...
    error::{
        MethodNotAllowedOnlyError, NotFoundOrUnexpectedApiError, OptionApiError,
    },
    routes::{
...
pub async fn project_budget_modify(
    ...
    if project_budget.year != year as u32 && !data.force {
        return Err(MethodNotAllowedOnlyError::MethodNotAllowedError(
            String::from("Changing past budgets not allowed"),
        )
        .into());
    }
    ...
    if data.amount.unwrap() <= cost.total as u32 && !data.force {
        return Err(MethodNotAllowedOnlyError::MethodNotAllowedError(
            String::from("Cost already exceeds desired budget amount"),
        )
        .into());
    }
